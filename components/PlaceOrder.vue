<template>
  <div id="print">
    <div
      ...
      class="max-h-[350px] w-full print:max-h-full print:overflow-visible overflow-hidden overflow-y-scroll rounded-md"
    >
      <div v-for="(item, index) in orderStore.uniqueOrderItems" :key="item.id">
        <div
        ...
        >
          <div
            :id="item.name"
            class="flex justify-between p-3 bg-SecondryColor hide w-full mb-2 rounded-md transition-all duration-700 scroll-smooth"
          >
            <div class="flex gap-2 justify-center items-center">
              <button
                @click="remove(item)"
                class="mr-2 bg-red w-5 h-5 rounded-full flex justify-center items-center text-sm"
              >
                X
              </button>
              <div
    ...
    <div>
      ...
      >
        <div v-if="orderStore.orderItems.length > 0" class="flex flex-col">
          ...
          <div
            class="flex justify-between print:justify-center print:gap-10 items-center"
          >
        ...
        <div
          v-else
          class="h-[500px] pt-12 mt-0 flex flex-col gap-2 justify-center items-center"
        >
          <div class="font-bold text-xl text-[#898a8a3c]">No Items</div>
          <Icon
            size="36px"
            name="mdi:archive-off-outline"
            class="text-[#898a8a3c]"
          ></Icon>
        </div>
        ...
        <div class="flex flex-col print:hidden">
          ...
          >
            <div
              v-if="show"
              class="font-semibold active:opacity-75 transition-all w-7 h-7 duration-300"
            >
              <img src="/doc.gif" alt="" />
            </div>
            <div v-else class="font-semibold transition-all duration-300">
              Place Order
            </div>
          </button>
        </div>
