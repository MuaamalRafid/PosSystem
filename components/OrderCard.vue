<template>
  <div class="" >
   ...
   class="card flex flex-col gap-4 relative   bg-SecondryColor h-40  py-2 px-4 rounded-md cursor-pointer group "
      >
      <div  :style="`background-color:${menuStore.Menu[menuStore.selectedIde].color}`" class=" z-0 top-0 left-0 absolute h-full  w-2 rounded-tl-md rounded-bl-md group-hover:w-full group-hover:rounded-md transition-all duration-500"></div>
...
const items = computed(()=>menuStore.Menu[menuStore.selectedIde].items);
